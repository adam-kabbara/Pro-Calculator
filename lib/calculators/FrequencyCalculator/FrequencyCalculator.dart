...
class FrequencyCalculatorPage extends StatelessWidget {
  static const String routeName = '/frequencyCalculator';

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: FrequencyCalculator()
    );
  }
}


class FrequencyCalculator extends StatefulWidget {
  @override
  _FrequencyCalculatorState createState() => _FrequencyCalculatorState();
}

class _FrequencyCalculatorState extends State<FrequencyCalculator> {
  String char;
  String dropdownValue1 = 'Hertz';
  String dropdownValue2 = 'Kilohertz';
  ...
  String calculate(String input, String fromValue, String toValue){
    ...
    else if (listEquals(transferUnits, ["Hertz", "Kilohertz"])){
      output = (decimalInput / Decimal.parse('1000')).toString();
    }
    else if (listEquals(transferUnits, ["Hertz", "Megahertz"])){
      output = (decimalInput / Decimal.parse('1e+6')).toString();
    }
    else if (listEquals(transferUnits, ["Hertz", "Gigahertz"])){
      output = (decimalInput / Decimal.parse('1e+9')).toString();
    }
    else if (listEquals(transferUnits, ["Kilohertz", "Hertz"])){
      output = (decimalInput * Decimal.parse('1000')).toString();
    }
    else if (listEquals(transferUnits, ["Kilohertz", "Megahertz"])){
      output = (decimalInput / Decimal.parse('1000')).toString();
    }
    else if (listEquals(transferUnits, ["Kilohertz", "Gigahertz"])){
      output = (decimalInput / Decimal.parse('1e+6')).toString();
    }
    else if (listEquals(transferUnits, ["Megahertz", "Hertz"])){
      output = (decimalInput * Decimal.parse('1e+6')).toString();
    }
    else if (listEquals(transferUnits, ["Megahertz", "Kilohertz"])){
      output = (decimalInput * Decimal.parse('1000')).toString();
    }
    else if (listEquals(transferUnits, ["Megahertz", "Gigahertz"])){
      output = (decimalInput / Decimal.parse('1000')).toString();
    }
    else if (listEquals(transferUnits, ["Gigahertz", "Hertz"])){
      output = (decimalInput * Decimal.parse('1e+9')).toString();
    }
    else if (listEquals(transferUnits, ["Gigahertz", "Kilohertz"])){
      output = (decimalInput * Decimal.parse('1e+6')).toString();
    }
    else if (listEquals(transferUnits, ["Gigahertz", "Megahertz"])){
      output = (decimalInput * Decimal.parse('1000')).toString();
    }
    return output;
  }
  ...
  Widget build(BuildContext context) {
    ...
    appBar: PreferredSize(
      ...
      child: AppBar(
        ...
        title: Text("Frequency Calculator", style: TextStyle(fontSize: displayHeight(context) * 0.025, color: Colors.white)),
        ),
      ),
    drawer: AppDrawer(),
            ...
            children: [
              ...
              Padding(
                ...
                child: DropdownButton(
                  ...
                  items: ['Hertz', 'Kilohertz', 'Megahertz', 'Gigahertz'].map<DropdownMenuItem<String>>((String value) {
